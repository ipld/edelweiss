...
func GreetingService_Client_WithHTTPClient(hc *pd5.Client) GreetingService_ClientOption {
...
func New_GreetingService_Client(endpoint string, opts ...GreetingService_ClientOption) (*client_GreetingService, error) {
	...
	c := &client_GreetingService{endpoint: u, httpClient: pd5.DefaultClient, unsupported: make(map[string]bool)}
	for _, o := range opts {
		if err := o(c); err != nil {
			return nil, err
		}
...
func (c *client_GreetingService) Identify_Async(ctx pd6.Context, req *GreetingService_IdentifyArg) (<-chan GreetingService_Identify_AsyncResult, error) {
	...
	buf, err := pd4.Encode(envelope, pd7.Encode)
	if err != nil {
...
func process_GreetingService_Identify_AsyncResult(ctx pd6.Context, ch chan<- GreetingService_Identify_AsyncResult, r pd8.ReadCloser) {
	defer close(ch)
	defer r.Close()
	for {
		var out GreetingService_Identify_AsyncResult

		n, err := pd4.DecodeStreaming(r, pd7.Decode)
		if pd9.Is(err, pd8.EOF) || pd9.Is(err, pd8.ErrUnexpectedEOF) {
			return
		}

		if err != nil {
			out = GreetingService_Identify_AsyncResult{Err: pd14.ErrProto{Cause: err}} // IPLD decode error
		} else {
			env := &AnonInductive5{}
			if err = env.Parse(n); err != nil {
				out = GreetingService_Identify_AsyncResult{Err: pd14.ErrProto{Cause: err}} // schema decode error
			} else if env.Error != nil {
				out = GreetingService_Identify_AsyncResult{Err: pd14.ErrService{Cause: pd9.New(string(env.Error.Code))}} // service-level error
...
func (c *client_GreetingService) Hello_Async(ctx pd6.Context, req *HelloRequest) (<-chan GreetingService_Hello_AsyncResult, error) {
	// check if we have memoized that this method is not supported by the server
	c.ulk.Lock()
...
func process_GreetingService_Hello_AsyncResult(ctx pd6.Context, ch chan<- GreetingService_Hello_AsyncResult, r pd8.ReadCloser) {
	defer close(ch)
	defer r.Close()
	for {
		...
		if err != nil {
			out = GreetingService_Hello_AsyncResult{Err: pd14.ErrProto{Cause: err}} // IPLD decode error
		} else {
			env := &AnonInductive5{}
			if err = env.Parse(n); err != nil {
				out = GreetingService_Hello_AsyncResult{Err: pd14.ErrProto{Cause: err}} // schema decode error
			} else if env.Error != nil {
				out = GreetingService_Hello_AsyncResult{Err: pd14.ErrService{Cause: pd9.New(string(env.Error.Code))}} // service-level error
...
func GreetingService_AsyncHandler(s GreetingService_Server) pd5.HandlerFunc {
	return func(writer pd5.ResponseWriter, request *pd5.Request) {
		...
		n, err := pd4.Decode(msg, pd7.Decode)
		if err != nil {
		...
		case env.Hello != nil:
			ch, err := s.Hello(pd6.Background(), env.Hello)
			if err != nil {
