...
import (
	...
	pd7 "github.com/ipld/edelweiss/services"
...
func (c *client_DelegatedRouting) Identify_Async(ctx pd8.Context, req *DelegatedRouting_IdentifyArg) (<-chan DelegatedRouting_Identify_AsyncResult, error) {
	...
	buf, err := pd6.Encode(envelope, pd13.Encode)
	if err != nil {
	...
	u := *c.endpoint
	httpReq, err := pd4.NewRequestWithContext(ctx, "POST", u.String(), pd12.NewReader(buf))
	if err != nil {
		return nil, err
	}
	...
	if resp.StatusCode != 200 {
		...
		if resp.Header != nil {
			if errValues, ok := resp.Header["Error"]; ok && len(errValues) == 1 {
				err = pd7.ErrService{Cause: pd2.Errorf("%s", errValues[0])}
			} else {
...
func process_DelegatedRouting_Identify_AsyncResult(ctx pd8.Context, ch chan<- DelegatedRouting_Identify_AsyncResult, r pd9.ReadCloser) {
	defer close(ch)
	defer r.Close()
	for {
		var out DelegatedRouting_Identify_AsyncResult

		n, err := pd6.DecodeStreaming(r, pd13.Decode)
		if pd11.Is(err, pd9.EOF) || pd11.Is(err, pd9.ErrUnexpectedEOF) {
			return
		}

		if err != nil {
			out = DelegatedRouting_Identify_AsyncResult{Err: pd7.ErrProto{Cause: err}} // IPLD decode error
		} else {
			env := &AnonInductive5{}
			if err = env.Parse(n); err != nil {
				out = DelegatedRouting_Identify_AsyncResult{Err: pd7.ErrProto{Cause: err}} // schema decode error
			} else if env.Error != nil {
				out = DelegatedRouting_Identify_AsyncResult{Err: pd7.ErrService{Cause: pd11.New(string(env.Error.Code))}} // service-level error
...
func process_DelegatedRouting_GetP2PProvide_AsyncResult(ctx pd8.Context, ch chan<- DelegatedRouting_GetP2PProvide_AsyncResult, r pd9.ReadCloser) {
	defer close(ch)
	defer r.Close()
	for {
		...
		if err != nil {
			out = DelegatedRouting_GetP2PProvide_AsyncResult{Err: pd7.ErrProto{Cause: err}} // IPLD decode error
		} else {
			env := &AnonInductive5{}
			if err = env.Parse(n); err != nil {
				out = DelegatedRouting_GetP2PProvide_AsyncResult{Err: pd7.ErrProto{Cause: err}} // schema decode error
			} else if env.Error != nil {
				out = DelegatedRouting_GetP2PProvide_AsyncResult{Err: pd7.ErrService{Cause: pd11.New(string(env.Error.Code))}} // service-level error
...
func DelegatedRouting_AsyncHandler(s DelegatedRouting_Server) pd4.HandlerFunc {
	return func(writer pd4.ResponseWriter, request *pd4.Request) {
		...
		n, err := pd6.Decode(msg, pd13.Decode)
		if err != nil {
		...
		case env.GetP2PProvide != nil:
			...
			for resp := range ch {
				...
				var buf pd12.Buffer
				if err = pd6.EncodeStreaming(&buf, env, pd13.Encode); err != nil {
